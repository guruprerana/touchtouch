import os
import sys
# Add sparsh directory to path so that we can import it
...
class VitForceEstimation:
    def __init__(self, args):
        ...
        self.optimizer = optim.AdamW(
            ...
            list(self.force_probe.parameters()), 
    ...
    def _init_model(self):
        ...
        try:
            ...
            self.force_probe = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Linear(num_features // 4, 3)  # 3 for force XYZ
            )
            
            logger.info("Model architecture modified with AttentivePooler for force estimation")
    ...
    def train_epoch(self, epoch):
        ...
        for i, batch in enumerate(tqdm(self.train_loader, desc=f"Epoch {epoch}")):
            ...
            logits = self.force_probe(pooled_features)  # Final prediction
            ...
            torch.nn.utils.clip_grad_norm_(self.force_probe.parameters(), max_norm=1.0)
    ...
    def evaluate(self, epoch=None):
        ...
        with torch.no_grad():
            for batch in tqdm(self.test_loader, desc="Evaluating"):
                ...
                pooled_features = self.attentive_pooler(features).squeeze(1)  # Apply attentive pooling
                logits = self.force_probe(pooled_features) * force_scale  # Final prediction
    ...
    def save_model(self, path):
        ...
        torch.save({
            ...
            'force_probe_state_dict': self.force_probe.state_dict(),
    ...
    def load_model(self, path):
        ...
        if os.path.exists(path):
            ...
            self.force_probe.load_state_dict(checkpoint['force_probe_state_dict'])
...
def parse_args():
    ...
    parser.add_argument("--wandb_run_name", type=str, default="vit-force-estimation-data0.01",
                        help="W&B run name")
